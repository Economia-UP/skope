...
def imports():
    ...
    SERIES_ID = '452056'  # Your INEGI series ID
    ...
    df.columns = ['fecha', 'Importaciones']
    
    df.sort_values(by='fecha', ascending=True, inplace=True)
    
    # df['Crecimiento importaciones'] = df['Importaciones'].pct_change(periods = 1)*100

    # Define the output directory and ensure it exists
    output_dir = 'MX'
    # os.makedirs(output_dir, exist_ok=True)  # Create the directory if it doesn't exist
    output_file = os.path.join(output_dir, 'ac_imports.csv')
